    ...
    public function index()
    {
        ...
        $dashboardData = DB::table('orders')
            ->selectRaw("
                ...
                SUM((CASE WHEN status='processing' THEN total_amount ELSE 0 END)+(CASE WHEN status='shipped' THEN total_amount ELSE 0 END)) AS TotalProcessingAmount,
                SUM((CASE WHEN status='delivered' THEN total_amount ELSE 0 END)+(CASE WHEN status='pending' THEN total_amount ELSE 0 END)) AS TotalDeliveredAmount,
                SUM((CASE WHEN status='canceled' THEN total_amount ELSE 0 END)+(CASE WHEN status='returned' THEN total_amount ELSE 0 END)) AS TotalCanceledAmount,
                COUNT(*) AS Total,
