                            ...
                            <tr>
                                <td>
                                    <div class="shopping-cart__product-item">
                                        <img loading="lazy" src="{{asset('storage/products_image/' . $item->model->image)}}" alt="{{$item->name}}" width="120" height="120" />
                    ...
                    <div class="cart-table-footer">
                        ...
                        @else
                            <form action="{{route('cart.remove.coupon')}}" method="POST" class="position-relative bg-body">
                                @csrf
                                @method('DELETE')
                                <input class="form-control" type="text" name="coupon_code" placeholder="Coupon Code" value="@if(\Illuminate\Support\Facades\Session::has('coupon')) {{\Illuminate\Support\Facades\Session::get('coupon')['code']}} @endif">
                                <input class="btn-link fw-medium position-absolute top-0 end-0 h-100 px-4" type="submit"
                                       value="REMOVE COUPON">
                            </form>
                        @endif

                        <form action="{{route('cart.clear',['rowId'=>$item->rowId])}}" method="POST" >
                            @csrf
                            @method('delete')
                                ...
                                <table class="cart-totals">
                                    ...
                                    <tr>
                                     ...
                                     <tr>
                                        ...
                                        <td>${{\Illuminate\Support\Facades\Session::get('discounts')['discount']}}</td>
                                    </tr>
                                    <tr>
                                        <th>Subtotal After Discount</th>
                                        <td>${{\Illuminate\Support\Facades\Session::get('discounts')['subtotal']}}</td>
                                    </tr>
                                    ...
                                    <tr>
                                        <th>TAX</th>
                                        <td>${{\Illuminate\Support\Facades\Session::get('discounts')['tax']}}</td>
                                    </tr>
                                    <tr>
                                        <th>Total</th>
                                        <td>${{\Illuminate\Support\Facades\Session::get('discounts')['total']}}</td>
                                    </tr>
                                    </tbody>
                            @else
                            <table class="cart-totals">
        ...
        <script>
            $(function(){
                $(".qty-control__increase").on("click", function(){
                    $(this).closest('form').submit(); // تحديث الكمية عند النقر على زر "+"
                });
